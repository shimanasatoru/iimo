        ...
        <section id="content" class="col-12">
          ...
          <div class="card">
            <div class="card-header row align-items-center">
              <div class="col-auto">
                <div class="input-group input-group-sm">
                  <div class="input-group-prepend">
                    <button type="button" class="modal-url btn btn-sm btn-secondary" data-id="modal-1" data-title="テーマ作成/変更" data-footer_class="mf-edit-theme" data-url="{$smarty.const.ADDRESS_CMS}templateModule/theme/edit/?theme={$smarty.request.theme|default:null} #content">
              ...
              <div class="col-auto">
                <button type="button" class="modal-url btn btn-sm btn-primary" data-id="modal-editor" data-title="/{$smarty.request.directory|default:null}" data-url="{$smarty.const.ADDRESS_CMS}templateModule/edit/?theme={$smarty.request.theme|default:null}&directory={$smarty.request.directory|default:null} #content" {if !$smarty.request.theme|default:null}disabled{/if}>
                  ファイルの新規作成
                </button>
              </div>
            </div>
...
<script>
  ...
  $(document).on('click','.theme-entry, .theme-delete', function() {
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS +'templateModule/push/?type=theme&dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  $(document).on('click','.template-entry, .template-delete', function() {
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS +'templateModule/push/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  function themeGet(){
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'templateModule/get/?levelStop=1&dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: themeView
  ...
  function directoryGet(){
    ...
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'templateModule/get/?theme='+ theme +'&dataType=json',
        data: null,
        dataType: 'json'
      },
  ...
  function filesGet(){
    ...
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'templateModule/get/?theme='+ theme +'&directory='+ directory +'&outputType=file&dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: filesView
    ...
    for(var i in row){
      ...
      $('<tr>'+
          ...
          '<td class="d-lg-flex justify-content-between">'+
            '<a href="#" class="modal-url font-weight-bold" data-id="modal-editor" data-title="/'+ directory +'" data-url="'+ ADDRESS_CMS +'templateModule/edit/?theme='+ theme +'&directory='+ directory +'&file='+ row[i].basename +' #content">'+
              row[i].basename +
            '</a>'+
