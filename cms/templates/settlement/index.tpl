    ...
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          ...
          <p class="blockquote-footer mb-0">決済方法（手数料）を設定します。</p>
        </div>
        <div class="col-sm-6">
        ...
        <section id="content" class="col-12">
          ...
          <div class="card">
            <div class="card-header row align-items-center">
              <div class="col-auto">
                <button type="button" class="modal-url btn btn-sm btn-primary" data-id="modal-1" data-title="新規登録" data-footer_class="mf-edit-settlement" data-url="{$smarty.const.ADDRESS_CMS}settlement/edit/ #content">
                  <i class="fas fa-plus"></i>&nbsp;
                  新規登録
                </button>
                <div class="d-none mf-edit-settlement">
                  <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">閉じる</button>
                  <button type="button" class="settlement-delete btn btn-sm btn-danger">削除する</button>
                  <button type="button" class="settlement-entry btn btn-sm btn-primary">登録する</button>
                </div>
              </div>
            </div>
...
<script>
   ...
   * 登録・削除ボタン
   */
  $(document).on('click','.settlement-entry, .settlement-delete', function() {
    var fn_name = '登録';
    $('[name="delete_kbn"]').val('');
    if($(this).hasClass('settlement-delete')){
      fn_name = '削除'
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS +'settlement/push/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  $(document).on('sortupdate', '#data', function(){
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS + 'settlement/push/sort/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  function getData(){
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'settlement/get/?dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: display
  ...
  function display(d){
    ...
    for(var i in row){
      $('<tr data-id="'+row[i].id+'">'+
        '<td>'+
          '<div>'+
            '<a class="modal-url font-weight-bold" data-url="'+ADDRESS_CMS+'settlement/edit/'+row[i].id+'/ #content" data-id="modal-1" data-title="変更" data-footer_class="mf-edit-settlement" href="#">'+
