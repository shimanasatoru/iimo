        ...
        <section id="content" class="col-12">
          ...
          <div class="card">
            <div class="card-header row align-items-center">
              <div class="col">
                <a href="{$smarty.const.ADDRESS_CMS}product/edit/?{$smarty.server.QUERY_STRING}" class="btn btn-sm btn-primary">
...
<script>
  ...
  function getData(){
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'product/get/?dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: display
  ...
  function display(d){
    ...
    for(var i in row){
      ...
      $(row[i].category).each(function(i, e){
        if(e.name){
          category += "<span class=\"badge\">"+ e.name +"</span>";
        }
      });
      if(row[i].stock_status == 1){
        stock_status = "する";
      }
      ...
      $('<tr data-id="'+row[i].id+'">'+
        ...
        '<td>'+
          '<div>'+
            '<a class="font-weight-bold '+classes+'" href="'+ADDRESS_CMS+'product/edit/'+row[i].id+'/?{$smarty.server.QUERY_STRING}">'+
              '<i class="fas fa-angle-right"></i>&nbsp;'+
              row[i].name+
            '</a>'+
            '<div class="text-xs text-muted">'+icon+'&nbsp;'+category+'</div>'+
          '</div>'+
        '</td>'+
        '<td class="text-right">'+
