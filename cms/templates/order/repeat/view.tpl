          ...
          <tbody>
            ...
            <tr class="text-right">
              <td class="text-left d-flex">
                <div class="mr-1">
                  <a class="modal-url btn btn-outline-primary btn-xs" data-id="modal-2" data-title="品目の変更" data-footer_class="mf-edit-item" data-url="{$smarty.const.ADDRESS_CMS}order/editItem/{$data->id}/{$d->id}/{$p->id}/ #content" href="#?">
                    <i class="fas fa-pencil-alt fa-fw"></i>
                  </a>
                </div>
                <div>
                  <span>{$p->model}</span>
                  {$p->temperature_zone_badge|default:'_'}
                  <span>{$p->name}</span>
              ...
              <td>
                {if $p->tax_rate == 8}※{/if}
                {$p->unit_tax_price|default:0|number_format}
              </td>
              <td>
                {$p->quantity|default:0|number_format}
              </td>
              <td>
                {$p->unit_name|default:'個'}
              </td>
              <td>
                {$p->tax_price|default:0|number_format}円
              </td>
            </tr>
            {foreach $p->option_include as $o}
            {if $o->selected}
