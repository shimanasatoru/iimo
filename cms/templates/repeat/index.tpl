        ...
        <section id="content" class="col-12">
          ...
          <div class="card">
            <div class="card-header row align-items-center">
              <div class="col">
                <a href="{$smarty.const.ADDRESS_CMS}repeat/editRepeat/?{$smarty.server.QUERY_STRING}" class="btn btn-sm btn-primary">
...
<script>
  ...
  $(document).on('sortstop', '#data', function(){
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS + 'repeat/push/sort/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  function getData(){
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'repeat/getRepeat/?dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: display
  ...
  function display(d){
    ...
    for(var i in row){
      ...
      if(row[i].release == 0){
        ...
        classes = 'text-muted';
      }else if(row[i].release == 1){
        icon = '<span class="badge bg-primary">公開中</span>';
      }else if(row[i].release == 2){
        icon = '<span class="badge bg-warning">限定</span>';
        classes = 'text-warning';
      }
      if(!row[i].delivery_name){
        row[i].delivery_name = "ー";
      }
      
      $('<tr data-id="'+row[i].id+'">'+
        '<td>'+
          '<a class="btn btn-sm btn-primary btn-block" href="'+ADDRESS_CMS+'repeat/item/'+row[i].id+'/?{$smarty.server.QUERY_STRING}">'+
        ...
        '<td>'+
          '<div>'+
            '<a class="font-weight-bold '+classes+'" href="'+ADDRESS_CMS+'repeat/editRepeat/'+row[i].id+'/?{$smarty.server.QUERY_STRING}">'+
              '<i class="fas fa-angle-right"></i>&nbsp;'+
              row[i].name+
            '</a>'+
            '<div class="text-xs text-muted">'+icon+'&nbsp;'+
              row[i].first_shipping_date_class+
              row[i].first_shipping_date+
            '</div>'+
          '</div>'+
        '</td>'+
        '<td class="text-right">'+
          String(row[i].unit_price).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')+'<small>円</small>'+
        '</td>'+
        '<td>'+
          row[i].delivery_date_cycle_unit+
          row[i].delivery_date_cycle+
          row[i].delivery_week_cycle+
        '</td>'+
        '<td>'+
          row[i].shipping_date_cycle+'日前'+
        '</td>'+
        '<td>'+
          row[i].settlement_date_cycle+'日前'+
        '</td>'+
        '<td>'+
          row[i].cancel_skip_date_cycle+'日前'+
        '</td>'+
        '<td>'+
