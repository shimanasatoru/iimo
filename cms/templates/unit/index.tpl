    ...
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          ...
          <p class="blockquote-footer mb-0">単位を設定します。主に商品設定の数量単位として使用されます。</p>
        </div>
        <div class="col-sm-6">
        ...
        <section id="content" class="col-12">
          ...
          <div class="card">
            <div class="card-header row align-items-center">
              <div class="col">
                <button type="button" class="modal-url btn btn-sm btn-primary" data-id="modal-1" data-title="新規登録" data-footer_class="mf-edit-unit" data-url="{$smarty.const.ADDRESS_CMS}unit/edit/ #content">
                  <i class="fas fa-plus"></i>&nbsp;
                  新規登録
                </button>
                <div class="d-none mf-edit-unit">
                  <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">閉じる</button>
                  <button type="button" class="unit-delete btn btn-sm btn-danger">削除する</button>
                  <button type="button" class="unit-entry btn btn-sm btn-primary">登録する</button>
                </div>
              </div>
            </div>
...
<script>
   ...
   * 登録・削除ボタン
   */
  $(document).on('click','.unit-entry, .unit-delete', function() {
    var fn_name = '登録';
    $('[name="delete_kbn"]').val('');
    if($(this).hasClass('unit-delete')){
      fn_name = '削除'
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS +'unit/push/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  $(document).on('sortupdate', '#data', function(){
    ...
    var e = {
      params: {
        type: 'POST',
        url: ADDRESS_CMS + 'unit/push/sort/?dataType=json',
        data: form,
        dataType: 'json',
        processData: false,
        contentType: false
  ...
  function getData(){
    var e = {
      params: {
        type: 'GET',
        url: ADDRESS_CMS + 'unit/get/?dataType=json',
        data: null,
        dataType: 'json'
      },
      doneName: display
  ...
  function display(d){
    ...
    for(var i in row){
      $('<tr data-id="'+row[i].id+'">'+
        '<td>'+
          '<div>'+
            '<a class="modal-url font-weight-bold" data-url="'+ADDRESS_CMS+'unit/edit/'+row[i].id+'/ #content" data-id="modal-1" data-title="変更" data-footer_class="mf-edit-unit" href="#">'+
