...
class orderRepeatDeliveryRepository extends dbRepository {
  
  use \host\cms\entity\orderRepeatDeliveryEntity;
  ...
  public function get() {
    ...
    self::setFrom("order_repeat_delivery_tbl d 
    ...
    if($order_repeat_id = self::getOrderRepeatId()){
      self::setWhere("d.order_repeat_id = :order_repeat_id");
      self::setValue(":order_repeat_id", $order_repeat_id);
    }
    ...
    try {
      ...
      while($d = $stmt->fetch(\PDO::FETCH_OBJ)){
        ...
        $p = new orderRepeatItemRepository;
        $p->setSiteId($d->site_id);
        $p->setOrderRepeatDeliveryId($d->id);
        $d->item = $p->get()->row;
        $this->row[] = $d;
      }
  ...
  public function push(){
    ...
    if($order_repeat_id = $this->getOrderRepeatId()){
      $data->data->order_repeat_id = $order_repeat_id;
    }
    ...
    try {
      $query = $this->queryCreate($data->data, 'order_repeat_delivery_tbl');
  ...
  public function setPacking(array $data_product, array $data_delivery){
    ...
    foreach($data_product as $index_p => $p){
      ...
      foreach($p->total->by_tax_rate as $rate => $value){
      ...
      $total->tax_price += $price->tax_price;
      $total->notax_price += $price->prnotax_priceice;
      $total->tax += $price->tax;
