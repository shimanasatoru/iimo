...
class templateRepository extends dbRepository {
  
  use \entity\templateEntity;
  ...
  public function tree($directory, $outputType = null, $levelStop = null, $tree = array(), $level = 0){
    ...
    //サイト側のテンプレート保存先
    $omit_dirname = self::getDirectory().'/design/'.self::getDesignTheme().'/files';
    foreach ($list as $key => $d){
      $path = pathinfo($d);
      $dirname = str_replace(DIR_SITE, "", $path['dirname']);
   ...
   */
  public function get() {
    $site_directory = self::getDirectory();
    if($site_directory){
      //オリジナルテーマ
      $directory = DIR_SITE;
      $directory.= $site_directory.'/design';
    }else{
      //共通テーマ
      $directory = DIR_MODULE;
      $site_directory = "module";
      //self::set_message('ディレクトリが見つかりません。');
      //return $this;
    }
  ...
  public function push(){
    ...
    if(!$push['site_directory'] || !$push['theme'] || !$push['directory']){
    ...
    try {
      ...
      $dir = DIR_SITE."{$push['site_directory']}/design/{$push['theme']}/files/{$push['directory']}/{$push['name']}";
      if(!$push['delete_kbn']){
  ...
  public function themePush(){
    ...
    unset($push['token']);
    try {
      $newDir = DIR_SITE."{$push['site_directory']}/design/{$push['name']}";
      $oldDir = DIR_SITE."{$push['site_directory']}/design/{$push['theme']}";
      if(!$push['delete_kbn']){
        if($push['theme']){
